...
class Table extends Component {
  ...
  componentDidMount() {
    axios.get(`https://ga-fpt.firebaseio.com/active.json`).then((res) => {
  ...
  componentWillUpdate() {
    ...
    axios.get(`https://ga-fpt.firebaseio.com/active.json`).then((res) => {
      ...
      stocks != null && (count = Object.keys(stocks).length);
  ...
  render() {
    return (
      <div className="MyStocks" id="mystocks">
        ...
        <table className="MyStocksTable">
          ...
          <tbody>
            ...
            {this.state.data == null ? (
              <tr>
                <td>
                  <h3>NO STOCKS HAVE BEEN SELECTED</h3>
                </td>
              </tr>
            ) : (
        ...
        {this.state.count > 5 && (
          <h3>WARNING, Table should contain not more than 5 Stocks!</h3>
        )}
      </div>
    );
  }
